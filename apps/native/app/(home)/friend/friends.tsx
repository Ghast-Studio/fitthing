            ...
            <View style={styles.friendRow}>
              ...
              {friend.odataActiveWorkout && (
                <Button
                  variant="secondary"
                  size="sm"
                  onPress={() => router.push(`/workout/spectate/${friend.odataActiveWorkout}` as any)}
                >
                  <Ionicons name="eye-outline" size={16} color={foregroundColor} />
...
export default function FriendsPage() {
  ...
  const pendingRequests = useQuery(api.friends.getRequests, user?.id ? { userId: user.id } : "skip");
